...
for height in range(100000, 700001, 100000):
    ...
    collection = db[f'linked-inputs-{height}-strict']
        ...
        if inputs:
            ...
            for address in inputs:
                ...
                if matching:
                    ...
                    old_list = matching["aggregated_inputs_strict"]
                    if set(old_list) != set(matching):
                        ...
                        try:
                            collection.update_one({
                                ...
                                '$set': {
                                    'aggregated_inputs_strict': old_list
                                }
                            }, upsert=False)
            ...
            if not exists:
                try:
                    collection.insert_one(
                        {"_id": inputs[0], "aggregated_inputs_strict": inputs, "tx_hashes": [transaction["tx_hash"]]})
                    collection.create_index("aggregated_inputs_strict")
                except pymongo.errors.WriteError as e:
        ...
        if inputs:
            for address in inputs:
                ...
                if not matching:
                    try:
                        collection.insert_one(
                            {"_id": inputs[0], "aggregated_inputs_strict": inputs[0], "tx_hashes": [transaction["tx_hash"]]})
                        collection.create_index("aggregated_inputs_strict")
                    except pymongo.errors.WriteError as e:
                        print("writeError, insert")
                        print(transaction["_id"])
