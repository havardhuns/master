...
def h1_1_d1():
    for block_height in range(100000, 700001, 100000):
            ...
            transactions_common_input = transactions_collection.find({"$and": [{"height": {"$gt": block_height-50000, "$lt": block_height+50000}}, {"coinbase": False}]})
            transactions_common_input = [x for x in transactions]
...
def h1_2_d1():
    for block_height in range(100000, 700001, 100000):
            transactions_collection = db["data-set-1"]
            transactions = transactions_collection.find({"$and": [{"height": {"$gt": block_height-50000, "$lt": block_height+50000}}, {"coinbase": False}]})
            transactions = [x for x in transactions]
            entities_collection = db[f'z-linked-inputs-{block_height}-2']
            common_input_address_clustering(transactions, entities_collection)
