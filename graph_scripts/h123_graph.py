...
for block_height in range(100000, 700001, 100000):
    if block_height != 200000:
        ...
        h12 = otc_data_set_1_collection.count_documents({ "$and": [{"block_height": { "$gt": block_height - 50000, "$lt": block_height + 50000}}, {"$or": [{"heuristics.1": True}, {"heuristics.2": True}]}]})
        ...
        blocks[str(block_height)] = [h1, h2, h3, h12, h4]

def sum_lists(first, second):
    return [x + y for x, y in zip(first, second)]

labels = ["H2.1", "H2.2", "H2.3", "H2.1+H2.2", "H2.4"]
fig, ax = plt.subplots()
width = 0.35
bottom = [0,0,0,0,0]
...
plt.yticks([
    tick for tick in range(0, 18001, 2000)])
plt.legend()
plt.show()