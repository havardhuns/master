...
plt.bar(r, _6, color = '#e60049',
        width = width, edgecolor = 'black',
        label='Other output not OTC addressed previously')
plt.bar(r + width, _7, color = '#00bfa0',
        width = width, edgecolor = 'black',
        label='Optimal change heuristic')
plt.bar(r + width*2, _8, color = '#ffa300',
        width = width, edgecolor = 'black',
        label='Number of inputs equals two')
plt.bar(r + width * 3, _9, color = '#e6d800',
        width = width, edgecolor = 'black',
        label='OTC value has more than four decimals in fractional part')
  
plt.xlabel("Block height")
plt.ylabel("Number of transactions with OTC address")

# plt.grid(linestyle='--')
plt.xticks(r + width*1.5,['~100k', '~300k', '~400k', '~500k', '~600k', '~700k', ])
plt.legend()
