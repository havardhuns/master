...
def has_not_been_otc_addressed_previously_h2_4(address):
    ...
    otc_output = next(output for output in transaction["outputs"] if output["address"][0] == address)
    if not otc_value_is_smaller_than_all_input_values( otc_output["value"]["value"], transaction["inputs"]):
        return True

    other_output =  next(output for output in transaction["outputs"] if output["address"][0] != address)
    first_transaction_for_other_address_hash = get_first_transaction_hash(other_output["address"][0])
...
def run_otc_heuristic(heuristic_function, transactions_collection, otc_collection):
    ...
    for transaction in tqdm(transactions):
        count = otc_collection.count_documents({"tx_hash": transaction["tx_hash"]})
        if count == 0:
            try:
...
if __name__ == '__main__':
    run_otc_heuristic(h2_123, db['data-set-1'], otc_collection = db['otc-addresses-data-set-1'])
    run_otc_heuristic(h2_4, db['data-set-2'], otc_collection = db['otc-addresses-data-set-2'])